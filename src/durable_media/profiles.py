...
class Profile:
 id:str; version:int; kind:str; width:int|None=None; height:int|None=None; video_codec:str|None=None; audio_codec:str|None=None; max_duration:float|None=None; max_bytes:int|None=None; fps:float|None=None; sample_rate:int|None=None; channels:int|None=None
 def fingerprint(self): return hashlib.sha256(json.dumps(asdict(self),sort_keys=True,separators=(",",":")).encode()).hexdigest()
 def to_dict(self):
  d=asdict(self); d["fingerprint"]=self.fingerprint(); return d
PROFILES={
...
def get_profile(name):
 ...
 except KeyError: raise ValueError("unknown profile")
