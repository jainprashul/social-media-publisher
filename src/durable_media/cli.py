...
def main(argv=None):
 ...
 x=sub.add_parser('publish'); x.add_argument('job'); x.add_argument('--dry-run',action='store_true')
 x=sub.add_parser('status'); x.add_argument('job')
 x=sub.add_parser('preview'); x.add_argument('job')
 ...
 elif a.cmd=='publish': out=publish(cfg,r,a.job,dry_run=a.dry_run)
 elif a.cmd=='status':
  j=r.job(a.job); out={'job':dict(j) if j else None,'approval':dict(r.approval(a.job)) if r.approval(a.job) else None,'receipt':dict(r.receipt(j['idempotency_key'])) if j and r.receipt(j['idempotency_key']) else None,'transitions':[dict(x) for x in r.transitions(a.job)]}
 elif a.cmd=='asset':
  if a.action=='show': out=dict(r.asset(a.asset))
  else:
   from .manifest import lineage; out=lineage(r,a.asset)
 elif a.cmd=='preview': out=preview(r,a.job)
